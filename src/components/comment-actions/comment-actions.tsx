...
export function CommentActions({
  ...
  const [optimisticLikedIds, setOptimisticLikedIds] =
    useState<string[]>(idsOfUsersWhoLiked);
  const [optimisticDislikedIds, setOptimisticDislikedIds] = useState<string[]>(
    idsOfUsersWhoDisliked,
  );
  const [isCommentReplyFormVisible, setIsCommentReplyFormVisible] =
    useState(false);

  const [commentRepliesData, setCommentRepliesData] = useState<CommentData[]>(
    [],
  ); // State to hold the data of the comment's replies

  const currentUserId = currentUserData?._id ?? null;

  const hasLiked = currentUserId
    ? optimisticLikedIds.includes(currentUserId)
    : false;
  const hasDisliked = currentUserId
    ? optimisticDislikedIds.includes(currentUserId)
    : false;
  ...
  const handleLike = async () => {
    ...
    if (hasLiked) {
      ...
      try {
        ...
        setOptimisticLikedIds(idsOfUsersWhoLiked);
        toast.error("Failed to remove like.");
      }
    }
    // If user is adding a like, we also need to check if they had previously disliked the comment. If so, we optimistically remove the dislike as well and make an API call to remove the dislike.
    else {
      // Add like (remove dislike if present)
      setOptimisticLikedIds((prev) => [...prev, currentUserId]);

      //   Remove dislike if it exists
      if (hasDisliked) {
        setOptimisticDislikedIds((prev) =>
          prev.filter((id) => id !== currentUserId),
        );
      }
      ...
      try {
        if (hasDisliked) {
          await fetch("/api/dislike-comment", {
            method: "DELETE",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ commentId, reviewId }),
          });
        ...
        setOptimisticLikedIds(idsOfUsersWhoLiked);
        setOptimisticDislikedIds(idsOfUsersWhoDisliked);
  ...
  const handleDislike = async () => {
    ...
    if (hasDisliked) {
      ...
      setOptimisticDislikedIds((prev) => [...prev, currentUserId]);
  ...
  const handleFetchCommentReplies = async () => {
    try {
      ...
      const commmentRepliesData = await response.json();

      setCommentRepliesData(commmentRepliesData);
    } catch (error) {
      toast.error("Failed to fetch comment replies.");
    }
  };

  const handleToggleCommentReplyFormVisibility = () => {
    setIsCommentReplyFormVisible((prev) => !prev);

    if (!isCommentReplyFormVisible && commentRepliesData.length === 0) {
      // Fetch replies data when the form is being shown for the first time
  ...
  return (
    <div>
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className={`flex items-center gap-1 transition-colors cursor-pointer ${
              hasLiked ? "text-green-500" : "text-gray-400 hover:text-green-500"
            }`}
          >
            <svg
            ...
            >
              {hasLiked ? (
                ...
                /* not-liked.svg – outline thumbs up */
          ...
          >
            ...
            >
              {hasDisliked ? (
                /* disliked.svg – filled thumbs down */
                ...
                /* not-disliked.svg – outline thumbs down */
          ...
          <button
            onClick={(event) => {
              ...
              handleToggleCommentReplyFormVisibility();
            }}
      ...
      {/* comment form modal */}
      {isCommentReplyFormVisible && (
        <div
          className="fixed inset-0 z-50 flex items-center justify-center bg-black/50"
          onClick={() => setIsCommentReplyFormVisible(false)}
        >
          <div
            className="relative w-full max-w-lg mx-4 bg-white dark:bg-zinc-900 rounded-xl shadow-xl flex flex-col max-h-[90vh]"
          ...
          >
            <div className="flex items-center justify-between px-4 py-3 border-b border-gray-200 dark:border-zinc-700">
              ...
              <button
                ...
                className="p-1 rounded-full text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 transition-colors cursor-pointer"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  viewBox="0 0 24 24"
            ...
            <div className="overflow-y-auto p-4">
              <CommentForm parentCommentId={commentId} reviewId={reviewId} />
            </div>
          </div>
        </div>
      )}
